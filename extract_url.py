...
def url_iterator(iterable):
    for line in iterable:
        ...
        if match:
            ...
            if murl.code == 200:
                if match_sub:
                    yield (parse_url_with_sub(match_sub), murl.code)
                else:
                    yield (parse_url(match,line), murl.code)
            else:
                yield (url, murl.code)
        #elif match_branch:
...
for (pid, file_path) in imap(shlex.split, sys.stdin):
    ...
    for (url_tuple, code) in url_iterator( line for line in imap(lambda x: x.replace('\302\240', ' '), lines)):
        found_match = True
        if code == 200:
            (url, directory) = url_tuple
